...
@router.post("/")
async def chat_endpoint(request_data: ChatRequest, request: Request, db: Session = Depends(get_database)):
    ...
    # ✅ Use topic to decide which service to call
    if request_data.topic.lower() == "weather":
        weather_data = await get_weather(request_data.message.strip())
        if "error" in weather_data:
            reply = f"⚠️ {weather_data['error']}"
        else:
            reply = f"🌤️ Weather in {weather_data['city']}: {weather_data['temperature']}°C, {weather_data['description']}"
    else:
